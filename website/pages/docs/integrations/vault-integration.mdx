...
Policies are configured per Namespace. We will apply the policy in the example above to each Namespace.

```shell-session
# Apply the policy to our engineering namespace
$ vault policy write -namespace=engineering nomad-server nomad-server-policy.hcl

# Apply the policy to our engineering/frontend namespace
...
```shell-session
# Create the token role with Vaule for engineering
$ vault write -namespace=engineering /auth/token/roles/nomad-cluster @nomad-cluster-role.json

# Create the token role with Vaule for engineering/frontend
$ vault write -namespace=engineering/frontend /auth/token/roles/nomad-cluster @nomad-cluster-role.json
```

The [Nomad agent Vault integration][config] congfiguration supports specifying a Vault Namespace, but since
we will be using multiple and have configured our policies it can be left blank so by default Nomad interacts with Vault in it's root Namespace.
...
Since allow_unauthenticated is set to false job submitters will need to get a sufficiently privileged token to use when submitting a job.
...
[ent]: /docs/integrations/#enterprise-configuration
